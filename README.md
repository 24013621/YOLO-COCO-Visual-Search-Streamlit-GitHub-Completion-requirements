...
User uploads image (Streamlit UI)
   ...
   PIL Image (RGB) → NumPy array
            │
            ▼
   YOLOv8n model.predict(image, conf=threshold)
            │
            ▼
   Bounding boxes + class labels + confidence scores
            │
            ▼
   Annotated image (result.plot())  +  Detections table
            │
            ▼
...
#   Y O L O - C O C O - V i s u a l - S e a r c h - S t r e a m l i t - G i t H u b - C o m p l e t i o n - r e q u i r e m e n t s  
 